...
export function StarRatingInput({
  ...
  const starRange = useMemo(() => Array.from({ length: max }, (_, index) => index + 1), [max]);
  ...
  const handleSelect = (nextValue: number) => {
    ...
    const finalValue = allowZero && nextValue === currentValue ? 0 : nextValue;
        ...
        {starRange.map((starValue) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              aria-label={`Calificacion ${starValue}`}
